...
use spool::cli::{
    add_stream, add_task, assign_task, claim_task, comment_task, complete_task, delete_stream,
    free_task, list_streams, list_tasks, reopen_task, show_stream, show_task, update_stream_cmd,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Comment { id, body, r#ref } => {
            let ctx = SpoolContext::discover()?;
            comment_task(&ctx, &id, &body, r#ref.as_deref())
        }
